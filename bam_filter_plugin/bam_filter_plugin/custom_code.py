#!/usr/bin/env python
""" MultiQC bam_filter plugin functions
...
config.bam_filter_plugin_version = get_distribution("multiqc_bam_filter_plugin").version


# Add default config options for the things that are used in MultiQC_NGI
def bam_filter_plugin_execution_start():
    ...
    log.info("Running bam_filter MultiQC Plugin v{}".format(config.bam_filter_plugin_version))
    ...
    if 'bam_filter/summary_file' not in config.sp:
        config.update_dict( config.sp, { 'bam_filter/summary_file': { 'contents': 'Summary (bam_filter):' } } )
