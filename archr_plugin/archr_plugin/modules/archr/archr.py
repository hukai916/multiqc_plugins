...
class MultiqcModule(BaseMultiqcModule):
    ...
    def __init__(self):
        ...
        ## for archr_create_arrowfiles module, the result folder names can be "archr_create_arrowfiles_sampleXXX" or "archr_create_arrowfiles_annotation_sampleXXX", that is why the "/" is removed for path matching purpose.
            ...
            with open(file_path, "r") as summary:
                for num, line in enumerate(summary):
                    if num >= line_num and "%)" in line:
                        text = text + line.strip() + "<br>"
        plot_figure(self, dir_n = "archr_filter_doublets/", endswith = ".jpg", img_width = "48%", name = "Filtering doublets (ArchR built-in)", description = text, helptext = "Count how many doublets are filtered out.")
